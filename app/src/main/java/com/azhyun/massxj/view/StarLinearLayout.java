...
public class StarLinearLayout extends LinearLayout implements OnClickListener {
    ...
    public StarLinearLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (attrs != null) {
            ...
            mMargin = (int) a.getDimension(R.styleable.star_margin, 5);
    ...
    private void init() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            star.setImageResource(R.drawable.pingfen_1);
            stars.add(star);
            addView(star, params);
    ...
    private void setStar(int level) {
        int i;
        for (i = 0; i < level / 2; i++) {
            stars.get(i).setImageResource(R.drawable.pingfen_1);
        }
//        if (level % 2 > 0) {
...
//        }
        for (; i < stars.size(); i++) {
            stars.get(i).setImageResource(R.drawable.pingfen_0);
        }
    }
